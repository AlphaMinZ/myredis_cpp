...
DBExecutor::DBExecutor(DataStore::ptr dataStore)
    ...
    m_cmdHandlers.emplace(CmdTypeExpire, std::bind(&DataStore::expire, m_dataStore.get(), std::placeholders::_1));
...
bool DBExecutor::validCommand(CmdType cmd) {
    auto it = m_cmdHandlers.find(cmd);
    return it != m_cmdHandlers.end();
}
...
void DBExecutor::run() {
    ...
    for(;;) {
        ...
        if(result_cmd->from_me) {
            auto it = m_cmdHandlers.find((*(result_cmd->result))->getCmd());
            if(it == m_cmdHandlers.end()) {
                (*(result_cmd->result))->getReceiver()->send(std::make_shared<ErrReply>("unknown command " + (*(result_cmd->result))->getCmd()));
                continue;
            }

            m_dataStore->expirePreprocess((*(result_cmd->result))->getArgs()[0]);
            auto& handlerPtr = it->second;
            (*(result_cmd->result))->getReceiver()->send();
            // (std::move(result_cmd->result))
        }
    }
}
